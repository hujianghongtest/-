...
critical_pins = {
    ...
    13: "lcd:D/C", 9: "lcd:LED",      # v2.0: BL=9
    3: "dht1:SDA / r_dht:2",
    1: "btn_up", 2: "btn_down", 5: "btn_ok"  # v2.0: DOWN=2
}
...
print("=" * 60)
checks = [
    # (代码宏名称, 电路图预期GPIO) — v2.0 GPIO1-13 Only
    ("TFT_CS",   10),   # 片选 — lcd:CS
    ("TFT_DC",   13),   # 数据/命令 — lcd:D/C
    ("TFT_RST",   7),   # 仿真用GPIO7 (实物: 软件复位)
    ("TFT_MOSI", 11),   # SPI数据 — lcd:MOSI
    ("TFT_SCK",  12),   # SPI时钟 — lcd:SCK
    ("TFT_BL",    9),   # 背光 — lcd:LED
    ("DHT_PIN",   3),   # 传感器数据 — dht1:SDA
    ("BTN_UP",    1),   # UP键 — btn_up:1.r (实物: ADC合并到GPIO1)
    ("BTN_DOWN",  2),   # DOWN键 — btn_down:1.r (实物: BAT_ADC)
    ("BTN_OK",    5),   # OK键 — btn_ok:1.r (实物: I2S_BCLK)
]
...
fw_checks = [
    # (宏名称, 期望GPIO, 说明) — v2.0 GPIO1-13 Only
    ("BTN_ADC_PIN",     1,  "ADC按键(3合1电阻分压)"),
    ("BAT_ADC_PIN",     2,  "电池电压 ADC"),
    ("I2C_SDA",          3,  "I2C数据线 — BME280+DS3231"),
    ("I2C_SCL",          4,  "I2C时钟线 — BME280+DS3231"),
    ("I2S_BCLK",         5,  "I2S共享位时钟 (INMP441+MAX98357)"),
    ("I2S_WS",           6,  "I2S共享字选择 (INMP441+MAX98357)"),
    ("I2S_SD_IN",        7,  "INMP441 麦克风数据输入"),
    ("I2S_DOUT",         8,  "MAX98357 扬声器数据输出"),
    ("TFT_BL_PIN",       9,  "TFT 背光 PWM"),
    ("TFT_CS",          10,  "SPI 片选"),
    ("TFT_MOSI",        11,  "SPI 主机数据输出"),
    ("TFT_SCK",         12,  "SPI 时钟"),
    ("TFT_DC",          13,  "SPI 数据/命令选择"),
]
