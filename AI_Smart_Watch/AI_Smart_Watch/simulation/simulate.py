...
class HardwareTestRunner:
    ...
    def test_5_button_input(self):
        ...
        try:
            ...
            # DOWN 键
            self.btn_down.press()
            assert self.esp.digitalRead(3) == 0, "DOWN pressed not LOW"
    ...
    def test_7_pin_conflict_check(self):
        ...
        pin_assignments = {
            ...
            2:  "BAT_ADC (电池电压ADC采集)",
            3:  "BTN_DOWN (按键下, 内部上拉)",
            4:  "I2S_SPK_SD (MAX98357 关断控制)",
            5:  "BTN_OK (按键确认, 内部上拉)",
            6:  "I2S_SPK_DIN (MAX98357 I2S数据)",
            7:  "I2S_MIC_SD (INMP441 麦克风数据)",
            8:  "I2S_SPK_BCLK (功放 I2S位时钟)",
            9:  "I2S_SPK_LRC (功放 I2S通道选择)",
            10: "TFT_CS (SPI 片选)",
            11: "TFT_MOSI (SPI 主机数据输出)",
            12: "TFT_SCK (SPI 时钟)",
            13: "TFT_DC (SPI 数据/命令选择)",
            14: "TFT_RST (SPI 复位)",
            15: "I2S_MIC_SCK (麦克风 I2S位时钟)",
            16: "I2S_MIC_WS (麦克风 I2S通道选择)",
            17: "I2C_SDA (I2C数据线 — BME280+DS3231)",
            18: "I2C_SCL (I2C时钟线 — BME280+DS3231)",
            21: "WS2812_LED (板载RGB LED)",
            47: "TFT_BL (TFT背光 PWM控制)",
        }
        ...
        special_notes = {
            0:  "strap — BOOT 按键, 低电平进入下载模式",
            2:  "strap — 影响启动电压, 已用作 ADC (通过分压电阻隔离, 安全)",
            5:  "strap — 影响启动电压, 已用作按键 (加下拉电阻可行)",
            12: "JTAG — TDI, 已用作 SPI SCK (仅影响 JTAG 调试, 不影响运行)",
            13: "JTAG — TCK, 已用作 TFT DC",
            14: "JTAG — TMS, 已用作 TFT RST",
            15: "JTAG — TDO, 已用作 I2S 麦克风时钟",
        }
