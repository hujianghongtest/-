/*
 * ╔══════════════════════════════════════════════════════════════╗
 * ║  AI Smart Watch — Rust 固件 (ESP32-S3-Zero)                 ║
 ...
 *
 * 【引脚分配 (20个GPIO, 与 Hardware_Design.md 完全一致)】
 *   参见 config 模块中的 PinConfig 常量定义
 *
 * 【版本历史】
...
mod config {
    /// ESP32-S3-Zero 的完整引脚分配
    /// 此处集中管理所有引脚号，修改硬件接线时只需改这里

    // --- TFT 显示屏 (SPI2 / FSPI) ---
    pub const TFT_MOSI: i32 = 11;   // SPI 主机数据输出 (Master Out)
    pub const TFT_SCLK: i32 = 12;   // SPI 时钟 (Serial Clock)
    pub const TFT_CS: i32 = 10;     // 片选 (Chip Select)
    pub const TFT_DC: i32 = 13;     // 数据/命令选择 (Data/Command)
    pub const TFT_RST: i32 = 14;    // 硬件复位 (Reset)
    pub const TFT_BL: i32 = 47;     // 背光 PWM (Backlight)

    // --- I2C 总线 (BME280 + DS3231 共用) ---
    pub const I2C_SDA: i32 = 17;    // I2C 数据线
    pub const I2C_SCL: i32 = 18;    // I2C 时钟线

    // --- BME280 地址 ---
    pub const BME280_ADDR: u8 = 0x76;  // SDO 接 GND → 0x76

    // --- I2S 麦克风 (INMP441) ---
    pub const I2S_MIC_SCK: i32 = 15;   // 位时钟 (Bit Clock)
    pub const I2S_MIC_WS: i32 = 16;    // 字选择 (Word Select)
    pub const I2S_MIC_SD: i32 = 7;     // 串行数据 (麦克风输出)

    // --- I2S 功放 (MAX98357) ---
    pub const I2S_SPK_BCLK: i32 = 8;   // 位时钟 (Bit Clock)
    pub const I2S_SPK_LRC: i32 = 9;    // 左右通道 (LRCK)
    pub const I2S_SPK_DIN: i32 = 6;    // 串行数据 (流向功放)
    pub const I2S_SPK_SD: i32 = 4;     // 关断控制 (HIGH=工作 LOW=静音)

    // --- 物理按键 (内部上拉, LOW=按下) ---
    pub const BTN_UP: i32 = 1;         // 上翻
    pub const BTN_DOWN: i32 = 3;       // 下翻
    pub const BTN_OK: i32 = 5;         // 确认

    // --- 电池电压检测 ---
    pub const BAT_ADC_PIN: i32 = 2;    // ADC1_CH1, 外部 2:1 电阻分压
    pub const BAT_DIVIDER_RATIO: f32 = 2.0;  // 分压比 R1:R2 = 100k:100k
...
type WatchDisplay<'a> = Display<
    SPIInterface<
        ...
        gpio::PinDriver<'a, gpio::Gpio13, gpio::Output>,
        gpio::PinDriver<'a, gpio::Gpio10, gpio::Output>,
    >,
    ST7789,
    gpio::PinDriver<'a, gpio::Gpio14, gpio::Output>,
>;
...
fn main() -> anyhow::Result<()> {
    ...
    // 背光先点亮 (便于观察后续初始化)
    let mut backlight = gpio::PinDriver::output(peripherals.pins.gpio47)?;
    ...
    // 按键
    log::info!("[INIT] Buttons (UP=1, DOWN=3, OK=5)...");
    let mut btn_up = gpio::PinDriver::input(peripherals.pins.gpio1)?;
    btn_up.set_pull(gpio::Pull::Up)?;
    let mut btn_down = gpio::PinDriver::input(peripherals.pins.gpio3)?;
    btn_down.set_pull(gpio::Pull::Up)?;
    let mut btn_ok = gpio::PinDriver::input(peripherals.pins.gpio5)?;
    btn_ok.set_pull(gpio::Pull::Up)?;

    // 电池 ADC
    log::info!("[INIT] Battery ADC (GPIO2)...");
    let mut bat_adc = adc::AdcDriver::new(peripherals.adc1)?;
    let mut bat_pin = adc::AdcChannelDriver::new(peripherals.pins.gpio2)?;

    // ── 4. 运行上电自检 ──
    let test_results = run_self_test(
        &mut display, &mut bme, &mut rtc,
        &btn_up, &btn_down, &btn_ok,
        &mut bat_adc, &mut bat_pin,
    )?;
